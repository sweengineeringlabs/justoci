...
//!
//! Module map:
//!   * [`error`]      — [`error::RegistryPullError`], the typed
//!                      failure surface (folds into
//!                      [`crate::error::CliError::RegistryPull`]).
//!   * [`ref_parser`] — strict parser for `host[:port]/repo:tag`
//!                      and `@sha256:<hex>` reference forms.
//!   * [`auth`]       — auth header resolution + 401-then-bearer
//!                      token dance per OCI Distribution §3.4.
//!   * [`pull`]       — the pull pipeline: manifest, blobs,
//!                      referrers, layout assembly, atomic
//!                      `index.json`-last commit.
//!
//! Public entry points:
//!
